 ...
 * within that time.
 */
class StampedeProtector implements KeyValueStoreInterface
{
    /**
     ...
     * @param integer                $sla Stampede protection time, in milliseconds
     */
    public function __construct(KeyValueStoreInterface $kvs, $sla = 1000)
    {
        $this->kvs = $kvs;
        $this->sla = $sla;
    }
    ...
    {
        return $this->kvs->deleteMultiple($keys);
    }
    ...
    {
        $collection = $this->kvs->getCollection($name);
    ...
    public function getMultiple(array $keys, array &$tokens = null)
    {
        ...
        $values = $this->kvs->getMultiple(\array_merge($keys, $stampedeKeys), $tokens);
        ...
        while (--$attempts > 0 && !empty($protected) && $this->sleep()) {
            $values = $this->kvs->getMultiple($protected, $tokens2);
    ...
    */
    ...
    {
        return $this->kvs->setMultiple($items, $expire);
    }
    ...
    protected function protect(array $keys)
    {
        ...
        foreach ($keys as $key) {
            ...
            */
            $success[$key] = $this->kvs->add($this->stampedeKey($key), '', $this->sla);
        }
